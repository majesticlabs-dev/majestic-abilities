...
Verify provider versions and CLI flags against current Backblaze and provider documentation before changing production infrastructure.
...
b2 bucket create my-bucket allPrivate \
  --lifecycle-rule '{"daysFromHidingToDeleting": 30, "fileNamePrefix": "logs/"}'
...
```bash
b2 ls b2://my-bucket
b2 ls --recursive b2://my-bucket/path/to/folder/
b2 file upload my-bucket local-file.txt remote/path/file.txt
b2 file upload --content-type "application/json" my-bucket data.json data.json
b2 file download b2://my-bucket/remote/path/file.txt local-file.txt
b2 file download b2id://<fileId> local-file.txt
b2 rm b2://my-bucket/path/to/file.txt
b2 rm --dry-run --recursive --versions b2://my-bucket/path/to/folder/
```
...
```hcl
resource "b2_bucket_file_version" "config" {
  bucket_id    = b2_bucket.storage.bucket_id
...
resource "b2_bucket_file_version" "data" {
  bucket_id = b2_bucket.storage.bucket_id
...
# Leave endpoint unset unless Backblaze support provides a native B2 API override.
```
