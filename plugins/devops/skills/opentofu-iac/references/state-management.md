...
# Write state to an already-protected random file and remove it on exit.
umask 077
state_file="$(mktemp)"
trap 'rm -f "$state_file"' EXIT
tofu show -json > "$state_file"
# Inspect only the required fields with jq; never print the full state.
```
...
1. **Backup state securely**: `umask 077; backup_file="$(mktemp)"; tofu state pull > "$backup_file"`
...
# Review a refresh-only plan before changing state
...
```bash
# Pull state into an already-protected random file
umask 077
state_file="$(mktemp)"
trap 'rm -f "$state_file"' EXIT
tofu state pull > "$state_file"

# Push only after validating the protected recovery file
tofu state push "$state_file"

# Force push is dangerous and requires separate explicit review.
# tofu state push -force "$state_file"
```
...
```bash
# Pull into an already-protected temporary file without printing secret-bearing state.
umask 077
state_file="$(mktemp)"
trap 'rm -f "$state_file"' EXIT
tofu state pull > "$state_file"
# Inspect only the required fields with jq or a protected local editor.
