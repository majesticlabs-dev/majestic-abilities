...
Hetzner Object Storage supports S3 lifecycle policies. Configure and inspect policies with a currently supported S3-compatible client or provider resource, and verify the exact rule schema against Hetzner's current Object Storage documentation before applying retention changes.
...
4. Save to 1Password:
   ...
   # Add access_key_id and secret_access_key as concealed fields in the 1Password app.
   # Do not pass either credential as a CLI assignment.
   ```
...
```bash
export TF_VAR_hcloud_token="$(op read 'op://myproject/hetzner-api/token')"
export TF_VAR_s3_access_key="$(op read 'op://myproject/production-hetzner-s3/access_key_id')"
export TF_VAR_s3_secret_key="$(op read 'op://myproject/production-hetzner-s3/secret_access_key')"
...
unset TF_VAR_hcloud_token TF_VAR_s3_access_key TF_VAR_s3_secret_key
```
