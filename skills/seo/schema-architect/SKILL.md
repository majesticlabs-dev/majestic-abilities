...
**Routing:**
- Use this skill for Google rich results, AI Overviews eligibility, and entity disambiguation in classic search.
- Use `ai-crawler-readiness` for HTTP-layer LLM visibility in ChatGPT, Claude, Perplexity, and Gemini chat.
- Use `llms-txt-builder` for AI navigation file content.

**Caveat:** Structured data helps Google's classic indexing and AI Overviews via search. There is no public evidence that current chat LLMs parse JSON-LD when answering, so do not treat schema as the primary citation lever.
