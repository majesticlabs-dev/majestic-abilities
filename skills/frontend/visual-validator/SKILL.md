...
```
DESIGN_SYSTEM_PATH = configured design-system path, when the project provides one
```
...
When screenshot or browser capabilities are available:

1. Inspect the rendered structure and accessibility tree when possible.
2. Capture the target area at the relevant viewport and state.
3. Keep data, theme, viewport, and interaction state consistent across comparisons.
4. Analyze the screenshot against stated goals.
5. Document observations factually before making judgments.
