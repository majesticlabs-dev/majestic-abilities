...
LOOP_RE = re.compile(r"\b(for|while|forEach|map|filter|reduce|some|every|find|findIndex)\b")
...
def scan_python(path: Path, root: Path, text: str) -> list[Finding]:
    try:
        ...
        return [parse_finding] + scan_text(path, root, text)
...
def scan_text(path: Path, root: Path, text: str) -> list[Finding]:
    ...
    for idx, line in enumerate(lines, start=1):
        ...
        loop_stack = [(level, line_no) for level, line_no in loop_stack if level < indent + 4]
